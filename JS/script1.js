...
const personalMovieDB = {
   ...
   writeYourGenres: function (likes) {
      for (let i = 0; i < 3; i++) {
         likes[i] = prompt(`Ваш любимый жанр под номером ${i+1}`, '')
         if (likes[i] != null  && likes[i]  != '' && likes.length < 50) {
            console.log('done');
         } else {
            console.log('error');
            i--;
      ...
      likes.forEach(element => console.log(element));
   },

   showMyDB: function (doza) {
         if (!doza.privat) {
            console.log(personalMovieDB);
         }
   },

   toggleVisibleMyDB: function (hugo) {
      if (!hugo ) {
         hugo = 'true';
      } else {
         hugo ='false';
